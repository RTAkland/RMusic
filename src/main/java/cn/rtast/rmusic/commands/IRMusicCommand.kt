                ...
                .then(CommandManager.literal("mute").executes { this.executeMute(it);1 }).then(
                    CommandManager.literal("login").requires { it.hasPermissionLevel(2) }.then(
                        CommandManager.literal("cellphone").then(
                            CommandManager.argument("phone-number", StringArgumentType.string())
                                .then(
                                    CommandManager.argument("phone-password", StringArgumentType.string())
                                        .executes {
                                            this.executePhoneLogin(
                                                it, StringArgumentType.getString(
                                                    it, "phone-number"
                                                ), StringArgumentType.getString(it, "phone-password")
                                            );1
                                        })
                        )
                    ).then(
                    ...
                    .executes { this.executeLogout(it);1 }).then(
                    CommandManager.literal("search")
                        .then(CommandManager.argument("keyword", StringArgumentType.string())
                            .executes { this.executeSearch(it, StringArgumentType.getString(it, "keyword"), 10);1 })
                        .then(CommandManager.literal("limit")
                            .then(CommandManager.argument("limit", IntegerArgumentType.integer(5)).executes {
                                this.executeSearch(
                                    it,
                                    StringArgumentType.getString(it, "keyword"),
